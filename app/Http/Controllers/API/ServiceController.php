...
class ServiceController extends Controller
{
    ...
    public function getServiceListDataByVendor($id)
    {

        $ven=Vendors::where('id',$id)->count();
        if($ven==0){
            return ['nodata' => $ven];
        ...
        $holidays = Holiday::all();
        $data = Vendors::where('id',$id)->select('offday_setting')->first();
        $offDays = [];
        if ($data) {
            $offDays = explode(',', $data->offday_setting);
        }
        $validServices = [];
        $today = date("Y-m-d");
        if (!empty($services)) {
            ...
            for ($time = $data->service_starts; $time < $data->service_ends;) {        
                ...
                foreach ($bookedSlot as $index => $bs) {
                   ...
                   foreach($bs as $key => $item){
                        ...
                        if (($time >= $slotStartTime && $time < $slotEndTime) || (($timeAndDuration >= $slotStartTime && $timeAndDuration < $slotEndTime))){
