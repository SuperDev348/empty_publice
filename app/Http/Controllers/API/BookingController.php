...
class BookingController extends Controller
{
    ...
    public function getBookingList(Request $request)
    {
        ...
        return ['datarows' => $allGetBookingList, 'count' => $getBookingList['count'], 'columnName' => $columnName, 'customDateId' => $customDateId,'total'=>$total];
    }
    ...
    public function bookingDetails($id, $return_data = false)
    {
        ...
        if(Auth::user()->is_admin){
            $booking = Booking::Join('services', 'bookings.service_id', '=', 'services.id')
                ...
                ->select('bookings.*', 'users.id as user_id', 'users.avatar', 'users.first_name', 'users.last_name', 'users.email', 'users.phone', 'services.title', 'services.price', 'services.service_duration_type', 'vendors.name as vendor','payments.paid_amount', 'payment_methods.title as method_title', 'payment_methods.id as payment_method_id', 'payments.id as payment_id')
                ->find($id);
