...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function invitedRegistration(Request $request, $token)
    {
        ...
        $vendor_id = $invitedUser->invited_as;
        ...
        $role_id = Role::select('id')->where('title','vendor')->first();
        if(is_null($role_id)){
        ...
        if (User::select('email')->where('email', $email)->exists()) {
            ...
            if (User::where('email', $email)->update(['first_name' => $first_name, 'last_name' => $last_name, 'token' => $token, 'password' => Hash::make($password), 'verified' => 1, 'vendor_id' => $vendor_id, 'role_id' => $role_id['id']])) {
                Invite::where('email', $email)->update(['token' => '']);
            }

        } else {

            if (User::create(['first_name' => $first_name, 'last_name' => $last_name, 'email' => $email, 'token' => $token, 'password' => Hash::make($password), 'verified' => 1, 'vendor_id' => $vendor_id, 'role_id' => $role_id['id']])) {
                Invite::where('email', $email)->update(['token' => '']);
            }
        }
    ...
    public function sendsms(Request $request)
    {
        ...
        $phone='0096892395921';
        $sendSmsText='hi hey';
        $x = SmsSender::sendSms($phone, $sendSmsText, $this->settingConfig);
        error_log('ooooooooooooooooooooooooo');
        print_r($x);
        return $x;
