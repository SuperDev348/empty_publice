...
class User extends Authenticatable
{
    ...
    protected $fillable = [
        'first_name', 'last_name', 'email', 'phone', 'phone_object', 'password', 'gender', 'cellphone', 'avatar', 'date_of_birth', 'verified', 'role_id','disabled', 'token', 'is_admin', 'notification_check', 'verification_code','vendor_id'
    ];
    ...
    public static function getUserLists($request)
    {
        ...
        if (!empty($searchValue)) {
            $query = User::select('users.id as id', 'users.first_name', 'users.last_name', 'users.email', 'vendors.name as vendors_name', 'users.is_admin', 'users.disabled')
                ->where('vendors.name', 'LIKE', '%' . $searchValue . '%')
                ->leftJoin('vendors', 'users.vendor_id', '=', 'vendors.id')
                ->where('vendors.name', 'LIKE', '%' . $searchValue . '%')
    ...
    public static function userWithVendor($id)
    {
        $user = User::leftJoin('vendors', 'users.vendor_id', '=', 'vendors.id')
            ->where('users.id', $id)
            ->select('users.id as id', 'users.first_name', 'users.last_name', 'users.email', 'vendors.name as vendors_name', 'users.is_admin', 'users.vendor_id', 'disabled')
            ->first();
