<template>
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ $t('lang.break_time') }}</h5>
        ...
        <div class="modal-body">
            ...
            <form v-else class="form-row">
                <div class="form-group col-lg-12">
                    ...
                    <label for="input-title">{{$t('lang.title') }}</label>
                ...
                <div class="form-group col-lg-6">
                    <label for="start_time">{{ $t('lang.start_time') }}</label>
                    <div>
                        <vue-timepicker id="start_time"
                                        class="form-control"
                                        :placeholder="$t('lang.start_time')"
                                        hide-clear-button
                                        name="startTime"
                    ...
                    <div v-if="isSubmit && submitData.start_time == timePickerDefaultValueFormat"
                        ...
                        <small class="text-danger">
                            {{ $t('lang.start_time_is_required') }}
                        </small>
                    </div>
                </div>
                <div class="form-group col-lg-6">
                    <label for="end_time">{{ $t('lang.end_time') }}</label>
                    <div>
                        <vue-timepicker id="end_time"
                                        class="form-control"
                                        hide-clear-button
                                        :placeholder="$t('lang.end_time')"
                                        name="endTime"
                    ...
                    <div v-show="isSubmit && submitData.start_time == timePickerDefaultValueFormat"
                         class="heightError">
                        <small class="text-danger">
                            {{ $t('lang.end_time_is_required') }}
                        </small>
                    </div>
                </div>

                <div class="form-group col-12">
                    <label class="d-block" for="enabled">{{ $t('lang.enabled') }}</label>
                ...
                <div class="col-12">
                    <button class="btn app-color mobile-btn" type="submit" @click.prevent="save()">{{$t('lang.save')}}</button>

                    <button class="btn cancel-btn mobile-btn" data-dismiss="modal" @click.prevent=""> {{$t('lang.cancel') }}</button>
                </div>
            </form>
        </div>
    </div>
