    ...
    <div>
        <div class="modal-layout-header">
            <div class="container-fluid p-0">
                <div class="row">
                    <div class="col-10">
                        <h4 class="m-0" v-if="id">{{ $t('lang.change_service_policy') }}</h4>
                        <h4 class="m-0" v-else>{{ $t('lang.service_policy') }}</h4>
                    </div>
        ...
        <div class="modal-layout-content" v-else>
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    <label for="select_service">{{ $t('lang.service') }}</label>
                    <select id="select_service" class="form-control" name="service_id" v-model="service_id"
                            ...
                            :data-vv-as="$t('lang.service_small')">
                ...
                <div class="form-group col-md-6">
                    <label>{{$t('lang.consider_children_for_price') }}</label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" v-model="consider_children_for_price"
                               ...
                               :data-vv-as="$t('lang.consider_children_for_price_small')"
                               :class="{ 'is-invalid': submitted && errors.has('consider_children_for_price') }"
                               value="Yes">
                        <label for="childen_for_price_yes" class="radio-button-label">{{$t('lang.yes') }}</label>


                        <div class="form-group col-md-12">
                            <label for="custom-content">{{$t('lang.description') }}</label>
                    ...
                    <div class="form-group col-md-3">
                        <label for="age_range">{{$t('lang.age_range') }}</label>
                        <select id="age_range" class="form-control" name="age_range" v-model="age_range"
                                v-validate="'required'"
                                :class="{ 'is-invalid': submitted && errors.has('age_range') }"
                                :data-vv-as="$t('lang.age_range_small')">

                            <option value disabled selected>{{ $t('lang.choose_one') }}</option>
                            <option v-for="(agerange, index) in sliceItems(0,19)" :value="index+1">{{$t('lang.ziro')
                                }} {{agerange}} {{$t('lang.years')}}
                            </option>
                    ...
                    <div class="form-group col-md-3">
                        <label for="percentage">{{$t('lang.percentage') }}</label>
