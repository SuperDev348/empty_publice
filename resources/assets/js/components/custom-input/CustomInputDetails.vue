<template>
    <div>
        <div class="row modal-layout-header">
            <div class="col-10">
                <h5 class="bluish-text no-margin" v-if="id">{{ $t('lang.edit_custom_field') }}</h5>
                <h5 class="bluish-text no-margin" v-else>{{ $t('lang.add_custom_field') }}</h5>
            </div>
            ...
            <div class="row margin-fix">
                <div class="row margin-fix">
                    <div class="input-field col s6">
                        ...
                        <label for="input-label" :class="{'active': error}">{{ $t('lang.label_name') }}</label>
                        <span class="helper-text" :data-error="$t('lang.required_label_field')"></span>
                    </div>
                    <div class="input-field col s6">
                        ...
                        <span v-if="errorInvalid.fieldType" class="red-text helper-text">{{ $t('lang.required_input_type') }}</span>
                    </div>
                    ...
                    <div class="col s12"
                        ...
                        <div class="input-field col s11">
                            ...
                            <label for="input-options">{{ $t('lang.add_options') }}</label>
                        </div>
                        <div class="col s1">
                    ...
                    <div class="col s12 custom-field-check-box">
                        <label for="custom-field-check">
                            <input type="checkbox" id="custom-field-check" v-model="enableField" value="1"
                                   name="field-enable"/>
                            <span>{{ $t('lang.enable_field') }}</span> <br>
                        </label>
                    </div>
                    <div class="col s12 custom-field-check-box">
                        <label for="custom-field-check1">
                            <input type="checkbox" id="custom-field-check1" v-model="showTable" value="1"
                                   name="field-enable"/>
                            <span>{{ $t('lang.show_in_table') }}</span> <br>
                        </label>
                    </div>
                    <div class="col s12 custom-field-check-box" v-if="table_name == 'booking'">
                        <label for="custom-field-check2">
                            <input type="checkbox" id="custom-field-check2" v-model="visibleToClient" value="1"
                                   name="field-enable"/>
                            <span>{{ $t('lang.visible_to_client') }}</span> <br>
                        </label>
                    </div>
                </div>
            </div>
                    ...
                    if (this.id) {
                        if (this.label && this.fieldType) {
                            ...
                            instance.axiosPost('/save-custom-field/' + instance.id, {
                                ...
                                function (error) {
                                    M.toast({html: instance.$t('lang.getting_problems'), classes: 'red lighten-1'});
                                    instance.setPreloader('load', 'hide');
                                }
                            )
                        }
