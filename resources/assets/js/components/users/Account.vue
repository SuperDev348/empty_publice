    ...
    <form>
        <div class="form-row margin-top">
            <div class="form-group col-12">
                <label for="new_password">{{ $t('lang.new_password') }}</label>
                ...
                <div class="heightError" v-if="submitted && $v.password.$error">
                    <small class="text-danger" v-if="!$v.password.required">
                        {{$t('lang.password_is_required')}}
                    </small>
                    <small class="text-danger" v-if="!$v.password.minLength">
                        {{$t('lang.password_must_be_at_least_6_characters')}}
                    </small>
                </div>
            </div>
            <div class="form-group col-12 margin-top">
                <label for="confirm_password">{{ $t('lang.confirm_password') }}</label>
            ...
            changePassword() {
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        this.postNewPassword('/updatePassword/' + this.user, {
                            password: this.password,
                            password_confirmation: this.password_confirmation,
                        })
                    }
