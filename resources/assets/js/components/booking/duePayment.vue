    ...
    <div>
        <div class="modal-layout-header">
            <div class="container-fluid p-0">
                <div class="row">
                    <div class="col-10">
                        <h4 class="m-0">{{ $t('lang.due_payment') }}</h4>
                    </div>
            ...
            <form v-else>
                <div class="form-row justify-content-center">
                    <div class="form-group col-12 col-md-6">
                        <p>{{ $t('lang.booking_id') }}: #{{ paymentDetails.id }}</p>
                        <p v-if="paymentDetails.status == 'confirmed'">
                            {{ $t('lang.status') }}:
                            <span class="booking-status bg-success ml-1">
                                {{ $t('lang.'+paymentDetails.status+'_')}}
                            </span>
                        </p>
                        <p v-if="paymentDetails.status == 'pending'">
                            {{ $t('lang.status') }}:
                            <span class="booking-status bg-warning ml-1">
                                {{ $t('lang.'+paymentDetails.status)}}
                            </span>
                        </p>
                        <p>
                            {{ $t('lang.bill') }}: {{ paymentDetails.booking_bill}}
                        </p>
                        <p class="mb-0">
                            {{ $t('lang.due') }}: {{ paymentDetails.payment_stat}}
                        </p>
                    </div>
                </div>
                <div class="form-row justify-content-center">
                ...
                <div class="form-row justify-content-center">
                    <div v-if="decimalSeparator == '.'" class="form-group col-12 col-md-6">
                        <label for="price">{{ $t('lang.payment_amount') }}</label>
                    ...
                    <div v-if="decimalSeparator == ','" class="form-group col-12 col-md-6">
                        <label for="priceComma">{{ $t('lang.payment_amount') }}</label>
                        <input v-model="dueClearAmount"
                               class="form-control"
                               name="price"
                               id="priceComma"
